...
const insertMovieGenres = (
  ...
  const filterGenreData = genreRows.filter(g => genres.indexOf(g.genre) != -1)
  return (`INSERT INTO ${MOVIE_GENRES} (movie_id, genre_id) VALUES`
    + filterGenreData.map(g => `(${movieId}, ${g.id})`).join(",")
  )
};
...
const insertMovieActors = (
  ...
  const filterActorData = actorRows.filter(a => actors.indexOf(a.full_name) != -1)
  return (`INSERT INTO ${MOVIE_ACTORS} (movie_id, actor_id) VALUES`
    + filterActorData.map(a => `(${movieId}, ${a.id})`).join(",")
  )
};
...
const insertMovieDirectors = (
  ...
  const filterDirectorData = directorRows.filter(d => directors.indexOf(d.full_name) != -1)
  return (`INSERT INTO ${MOVIE_DIRECTORS} (movie_id, director_id) VALUES` 
    + filterDirectorData.map(d => `(${movieId}, ${d.id})`).join(",")
  )
};
...
const insertMovieKeywords = (
  ...
  const filterKeywordData = keywordRows.filter(k => keywords.indexOf(k.keyword) != -1)
  return (`INSERT INTO ${MOVIE_KEYWORDS} (movie_id, keyword_id) VALUES` 
    + filterKeywordData.map(k => `(${movieId}, ${k.id})`).join(",")
  )
};
...
const insertMovieProductionCompanies = (
  ...
  const filteCompanyData = productionCompanyRows.filter(p => productionCompanies.indexOf(p.company_name) != -1)
  return (`INSERT INTO ${MOVIE_PRODUCTION_COMPANIES} (movie_id, company_id) VALUES` 
    + filteCompanyData.map(p => `(${movieId}, ${p.id})`).join(",")
  )
};
